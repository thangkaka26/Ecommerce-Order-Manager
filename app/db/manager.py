...
class _Ecommerce:
    ...
    def __init__(self, root):
        ...
        self.main_menu_frame.place(width=self.width/4, height=self.height/1.3, x=30, y=100)

        # MAIN BUTTONS (MAIN MENU OPTIONS)
        addBtn = tk.Button(self.main_menu_frame, text="Add", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                                   command=self.addOptions)
        addBtn.grid(row=0, column=0, padx=30, pady=20)

        updateBtn = tk.Button(self.main_menu_frame, text="Update", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                                   command=self.updateOptions)
        updateBtn.grid(row=1, column=0, padx=30, pady=20)

        removeCustomerBtn = tk.Button(self.main_menu_frame, text="Remove", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"), 
                                      command=self.removeOptions)
        removeCustomerBtn.grid(row=2, column=0, padx=30, pady=20)

        searchCustomerBtn = tk.Button(self.main_menu_frame, text="Search", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                                      command=self.searchOptions)
        searchCustomerBtn.grid(row=3, column=0, padx=30, pady=20)

        showAllBtn = tk.Button(self.main_menu_frame, text="Show All", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                               command=self.showAllOptions)
        showAllBtn.grid(row=4, column=0, padx=30, pady=20)
        
        viewsBtn = tk.Button(self.main_menu_frame, text="Views", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                             command=self.viewOptions)
        viewsBtn.grid(row=5, column=0, padx=30, pady=20)

        exitBtn = tk.Button(self.main_menu_frame, text="Exit", bd=3, relief="raised", bg="light gray", width=20, font=("Courier",20,"bold"),
                             command=self.exitWindow)
        exitBtn.grid(row=6, column=0, padx=30, pady=25)
    ...
    def tabOrderDetailsFunc(self):
        ...
        orderdet_table = ttk.Treeview(self.orderdet_frame, xscrollcommand=x_scroll.set, yscrollcommand=y_scroll.set,
                                          columns=("OrderID", "CustomerName", "ProductName", "OrderDate", "Quantity", "total_price", "Status"))
    ...
    def tabDateRevenueFunc(self):
        ...
        # CONNECTING TO DATABASE AND FETCHING ROWS
        self.dbConnect()
        self.cur.execute("SELECT * FROM revenue_to_date;")
        rows = self.cur.fetchall()
        self.connect_db.close()
